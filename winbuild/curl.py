    ...
    def build(self):
        ...
        with in_dir(os.path.join(curl_dir, 'winbuild')):
            ...
            with self.execute_batch() as b:
                b.add(
                    f"patch -p1 < {require_file_exists(os.path.join(self.bconf.winbuild_patch_root, 'libcurl-fix-zlib-references.patch'))}"
                )

                dll_or_static = 'dll' if self.use_dlls else 'static'
                extra_options = f' mode={dll_or_static}'
                if self.bconf.vc_version == 'vc9':
                ...
                if self.bconf.use_zlib:
                    ...
                    extra_options += f' WITH_ZLIB={dll_or_static}'
                if self.bconf.use_openssl:
                    ...
                    extra_options += f' WITH_SSL={dll_or_static}'
                if self.bconf.use_cares:
                    ...
                    extra_options += f' WITH_CARES={dll_or_static}'
                if self.bconf.use_libssh2:
                    ...
                    extra_options += f' WITH_SSH2={dll_or_static}'
                if self.bconf.use_nghttp2:
                    ...
                    extra_options += f' WITH_NGHTTP2={dll_or_static} NGHTTP2_STATICLIB=1'
                if self.bconf.use_libidn:
                    ...
                    extra_options += f' WITH_LIBIDN={dll_or_static}'
                ...
                extra_options += f' VC={self.bconf.vc_version[2:]}'
                ...
                b.add(f"nmake /f Makefile.vc {extra_options}")
        ...
        with in_dir(curl_dir):
            ...
            for dir in subdirs:
                if not dir.startswith(expected_dir):
                    raise Exception(f'{dir} does not start with {expected_dir}')
